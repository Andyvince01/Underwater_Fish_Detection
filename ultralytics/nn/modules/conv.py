...
class SPDConv(nn.Module):
    ...
    Example
    -------
    >>> spd_conv_block = SPDConv(in_channels=3, out_channels=64, kernel_size=3, scale=2, stride=1, debug=True)
    >>> input_tensor = torch.randn(1, 3, 32, 32)
    >>> output_tensor = spd_conv_block(input_tensor)    
    >>> print(output_tensor.shape)
    >>> # SpaceToDepth shape: torch.Size([1, 12, 16, 16])
...
class ResBlock_CBAM(nn.Module):
    def __init__(self, in_places, places, stride=1, downsampling=False, expansion=1):
        ''' This function initializes the ResBlock_CBAM block. 
        ...
        stride : int
            The stride value.
        downsampling : bool
            The downsampling flag.
        expansion : int
            The expansion factor.
        '''
        # Call the parent class constructor
        super(ResBlock_CBAM, self).__init__()
        # Define the expansion factor for the bottleneck block
    ...
    def forward(self, x : torch.Tensor) -> torch.Tensor:
        ''' This function performs the forward pass of the ResBlock_CBAM block.
        ...
        -------
        torch.Tensor
            The output tensor after performing the forward pass.
        '''
        # Store the input tensor
        residual = x
        # Perform the forward pass through the bottleneck block and the CBAM block
        out = self.bottleneck(x)
        out = self.cbam(out)
        # Perform the downsampling operation if required
        if self.downsampling:
            residual = self.downsample(x)
        # Add the residual tensor to the output tensor (skip connection)
        out += residual
