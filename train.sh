...
# ---- Default Parameters ----
BATCH_SIZE=(32)                                                        # Array of batch sizes to loop through
MODEL=(
    # "yolov8s"
    # "yolov8s-p2"
    # "yolov8s-p2-SPD"
    ...
    # "yolov8s-FishScale_UIEDM"
)    

WEIGHTS_VALUES=('yolov8s-FishScale-full.pt')
FREEZE_VALUES=(1)  # Default freeze value is 0 (no freezing)
...
# $PYTHON_BIN $TRAIN_SCRIPT --model $MODEL --weights 'best.pt' --kwargs epochs=50 batch=$BATCH_SIZE freeze="0" name="$MODEL-full" close_mosaic="5"
...
$PYTHON_BIN $TRAIN_SCRIPT --model $MODEL --weights 'yolov8s-FishScale-full.pt' --kwargs name="$MODEL-full@"
